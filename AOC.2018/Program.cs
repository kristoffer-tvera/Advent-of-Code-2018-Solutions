using System;

namespace AOC._2018
{
    internal static class Program
    {
        private static void Main(string[] args)
        {
//            Console.WriteLine($"Day 1, part 1: {Solutions.Day1Part1(DataImport.Day1Data())}");

//            Console.WriteLine($"Day 1, part 2: {Solutions.Day1Part2(DataImport.Day1Data())}");

//            Console.WriteLine($"Day 2, part 1: {Solutions.Day2Part1(DataImport.Day2Data())}");

//            Console.WriteLine($"Day 2, part 2: {Solutions.Day2Part2(DataImport.Day2Data())}");
            
//            Console.WriteLine($"Day 3, part 1: {Solutions.Day3Part1(DataImport.Day3Data())}");
            
//            Console.WriteLine($"Day 3, part 2: {Solutions.Day3Part2(DataImport.Day3Data())}");
            
            Console.WriteLine($"Day 4, part 1: {Solutions.Day4Part1(DataImport.Day4Data())}");
            
//            Console.WriteLine($"Day 4, part 2: {Solutions.Day4Part2(DataImport.Day4Data())}");
        }
    }
}